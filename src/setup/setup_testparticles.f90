...
use io,             only:master
use externalforces, only:iext_star,a,accradius1,accradius1_hard
use metric,         only:charge_metric => charge
use eos,            only:ieos
use physcon,        only:pi
...
integer :: i,ierr
real    :: dr,h0,xyz0(3),rhat(3),r2,vcirc,rtan(3),period,r0,fac,omega,rkerr,z,m1
...
m1 = 1.0 ! Default central mass

! Calculate orbit parameters based on orbit type
select case(orbtype)
case(1,3,4) ! circular, epicycle, vertical oscillation
   if (gr .and. imetric == imet_rn) then
      omega = sqrt(max(0., m1/r**3 - charge**2/r**4))
   else
      omega = 1./(r**(1.5)+spin)
   endif
   x0 = sqrt(r**2 + spin**2)
   vy0 = x0*omega
   period = 2.*pi/(omega + epsilon(omega))
...
case(0) ! custom
   ! Values should have been read from setup file or interactive
end select
